...
- Hostowanie: `SteamLobby` wymusza limit `RoundEngine.MaxPlayers` (obecnie 8), następnie `CreateLobby` (friends-only, ten sam limit) → `HostAddress` w danych lobby → `StartHost()`.
