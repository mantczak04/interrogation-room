...
Statusy: `Open` → `In progress` → `Review` (czeka na akceptację użytkownika) → `Done` (zmergowane do `main`). Dodatkowo `Blocked` z powodem.
...
| 1. Art direction | [FAZA-1-art-direction.md](./FAZA-1-art-direction.md) | `gfx/faza-1-art-direction` | In progress |
...
| 4. Post-processing | [FAZA-4-postprocessing.md](./FAZA-4-postprocessing.md) | `gfx/faza-4-postprocessing` | Open |
...
2. **Branch:** utwórz branch fazy z aktualnego `main` (nazwa z tabeli powyżej). Cała praca fazy odbywa się na tym branchu. Nie commituj do `main`.
3. **Statusy:** przy rozpoczęciu pracy zmień status fazy na `In progress` w tej tabeli ORAZ w nagłówku dokumentu fazy (jeden commit). Po ukończeniu wszystkich zadań i weryfikacji ustaw `Review`. `Done` ustawia wyłącznie użytkownik po merge.
